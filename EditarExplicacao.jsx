...
const EditarExplicacao=()=>{
    ...
    useEffect(()=>{
        if(id){
            axios.get(`http://localhost:9090/explicacoes/${id}`)
            .then (response =>{
                const explicacao=response.data
                setData(new Date(explicacao.data))
                const[hora, minuto]=explicacao.horaInicio.split(":")
                const inicio=new Date()
                inicio.setHours(hora,minuto)
                setHoraInicio(inicio)

                const [hFim,mFim]=explicacao.horaFim.split(":")
                const fim=new Date()
                setHoraFim(fim)
    ...
    const handleSubmit= async (e)=>{
        ...
        try{
       const response= await axios.put(`http://localhost:9090/explicacoes/${id}`,{
            explicandoId,
