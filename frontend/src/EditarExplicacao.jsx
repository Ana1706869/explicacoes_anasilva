...
const EditarExplicacao=()=>{
    ...
    useEffect(()=>{
        if(id){
            axios.get(`${process.env.REACT_APP_SERVER_URI}/explicacoes/${id}`)
            .then (response =>{
                const explicacao=response.data
                setData(new Date(explicacao.data))
                
                console.log(explicacao.horaInicio)
                
                
                setHoraInicio(new Date(`1970-01-01 ${explicacao.horaInicio}`))
                console.log(new Date(`1970-01-01 ${explicacao.horaInicio}`))
                setHoraFim(new Date(`1970-01-01 ${explicacao.horaFim}`))
                console.log(new Date(`1970-01-01 ${explicacao.horaFim}`))
                

                
                console.log(explicacao.horaFim)
                
                
                
                

                setLinguagem(explicacao.linguagem)
                setTipo(explicacao.tipo)
                setPagamento(explicacao.pagamento)
                setExplicandoId(explicacao.explicandoId)
            })
            .catch(error=>{
                console.error("Erro ao carregar explicação:",error)
                setErro("Erro ao carregar explicação!")
            })
        }
    },[id])
    ...
    const handleSubmit= async (e)=>{
        ...
        try{
       const response= await axios.put(`${process.env.REACT_APP_SERVER_URI}/explicacoes/${id}`,{
            explicandoId,
        ...
        return(<div >
            ...
            <DatePicker className="datePicker" selected={data} onChange={(date)=>setData(date)} dateFormat="yyyy-MM-dd" minDate={new Date()}/>
              ...
              <DatePicker className="datePicker" selected={horaInicio} onChange={(time)=>setHoraInicio(time)}
                ...
                <input  type="text" value={horaFim instanceof Date && ! isNaN(horaFim)? horaFim.toLocaleTimeString("en-US",{hour:"2-digit", minute:"2-digit"}):""}
                readOnly/>
                <br/>
                <br/>
...
export default EditarExplicacao